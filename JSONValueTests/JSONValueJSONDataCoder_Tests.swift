...
class JSONValueJSONDataCoder_Tests: XCTestCase {
    ...
    func testDecodeJSONValue_decodesCorrectly() {
        ...
        guard case .String(let watValue) = rootArray[0].value else {
        ...
        guard case .Int(let fiveValue) = rootArray[1].value else {
        ...
        guard case .Dictionary(let dictionaryValue) = rootArray[2].value else {
            XCTFail("Decoding generated an incorrect third element")
            return
        }

        guard case .Double(let fooValue)? = dictionaryValue["foo"]?.value else {
        ...
        guard case .Bool(let bazValue)? = dictionaryValue["baz"]?.value else {
